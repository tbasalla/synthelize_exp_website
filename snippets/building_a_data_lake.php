<section id="blog" class="blog">
      <div class="container">
            <article class="entry entry-single">
              ...
              <div class="entry-img">
                <img src="../assets/img/building-a-data-lake.jpg" alt="Building A Data Lake Image" class="img-fluid">
              </div>
              <h2 class="entry-title">Building a Data Lake</h2>

              <div class="entry-content">
<p>There are several steps involved in the process of building a data lake.  These steps are Locate, Ingest, Transform, and Recycle.</p>
<h4>Locate</h5>
<p>Before you get started in building a data lake you have to decide where the data lake will actually reside.  This usually entails a choice between on premises data storage and cloud data storage.  Whatever the choice your data store needs to be scalable, available, and performant, secure, and redundant.</p>
<h4>Ingest</h5>
<p>Once you have decided where your data lake will be located it is time to begin “loading” data into the data lake.  We prefer the term “ingest” over the term “load” because “loading” often carries with it connotations of the Extract/Transform/Load process familiar to data warehouses.  The process of loading a data warehouse involves a lot of data preparation, but data lakes ingest data in its most raw form.</p>
<p>This data ingestion can take many forms:</p>
<ul>
...
<h4>Transform</h5>
<p>Once the data is in the data lake, the data is often transformed into other formats and used to load tools more specifically targeted to the usefulness of the data lake data.  There are several common use cases for the data in a data lake; these use cases are outlined below:</p>
<p><b>Low Latency Search</b> – Often data from data lakes is in document form and an organization needs a way to search this information quickly.  These documents are stored in their base form in the data lake and then imported into search tools such as Elastic.</p>
<p><b>Data Warehousing</b> – Data lake information is often used to feed data warehouses.  In our first article we discussed the difference between data lakes and data warehouses.  Data lakes are NOT a replacement for data warehouses at most levels.  Instead, the data lake sits in front of at least one (sometimes numerous) data warehouses and data marts that draw data from the data lake.  Traditional data cleansing procedures are performed on the data as it is copied from the data lake to the target data warehouse.  </p>
<p><b>Data Visualization/Dashboards</b> – Data lake data is often fed into specialized data visualization systems that are then used to build complex dashboards and data visualizations.  A number of tools exist to visualize this data.</p>
...
<h4>Beats and Logstash</h4>
<p>Elastic offers Beats and Logstash as easy ways to move data into the Elastic Search engine.  These tools function as easy to configure services and monitors.  These monitors run on your servers, take high frequency data such as server logs and network traffic, and send them to Elasticsearch.</p>
<h4>Kibana</h4>
<p>Kibana is the data visualization arm of the elastic family.  It allows for a user of Elastic to create dashboards based on queries run against the backend Elasticsearch store.  Kibana is used frequently in server monitoring and tracking web hits for various properties, but it can be configured to provide dashboards of all sorts of data.</p>
<h4>TensorFlow and PyTorch</h3>
<p>TensorFlow and PyTorch are machine learning frameworks that allow data scientists to build machine learning models with ease.  </p>
<p>In our <a href="../data-lake-cloud-options">next article</a> we will discuss some of the most common tools used to work with data lakes.  We will also look at several cloud vendors and their approaches to building a data lake.</p>
</div>
